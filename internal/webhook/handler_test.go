...
// PostRegistration tests - returns metadata_public in response

func TestPostRegistrationSuccess(t *testing.T) {
	stub := resolverStub{
	...
	req := httptest.NewRequest(http.MethodPost, "/webhooks/kratos/post-registration", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()

	handler.PostRegistration(rec, req)
	...
	require.NoError(t, err)
	require.NotNil(t, resp.Identity.MetadataPublic)
	require.Equal(t, testActorID, resp.Identity.MetadataPublic.AlkemioActorID)
	require.Equal(t, testAgentID, resp.Identity.MetadataPublic.AlkemioAgentID)
}

func TestPostRegistrationValidatesActorIDUUID(t *testing.T) {
	stub := resolverStub{
...
func TestPostRegistrationNotFoundReturns500(t *testing.T) {
	stub := resolverStub{
		resolve: func(_ context.Context, _ string) (*alkemio.IdentityMapping, error) {
...
func TestPostRegistrationMalformedJSONReturns400(t *testing.T) {
	stub := resolverStub{
		resolve: func(_ context.Context, _ string) (*alkemio.IdentityMapping, error) {
			t.Fatal("resolver should not be called for malformed requests")
			return nil, nil
		},
	}
...
func TestPostRegistrationMissingIdentityIDReturns400(t *testing.T) {
	stub := resolverStub{
		resolve: func(_ context.Context, _ string) (*alkemio.IdentityMapping, error) {
